...
const init = (() => {
  ...
  async function getCurrentPosition(curPos) {
    ...
    const curCountry = await _dataHandler__WEBPACK_IMPORTED_MODULE_1__.apiCalls.getCoords(latitude, longitude);
    apiCords(curCountry);
  }
  ...
  async function apiCords(country) {
    try {
      await _dataHandler__WEBPACK_IMPORTED_MODULE_1__.apiCalls.getPlaceCoords(country);
      const getWeatherDaysArr = _dataHandler__WEBPACK_IMPORTED_MODULE_1__.apiCalls.getArray();
      _viewHandler__WEBPACK_IMPORTED_MODULE_2__.UI.handleDisplay(getWeatherDaysArr, country);
    } catch (err) {
      return err;
    }
  }
