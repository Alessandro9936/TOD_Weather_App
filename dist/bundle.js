...
const init = (() => {
  ...
  async function getCurrentCity(lat, lon) {
    try {
      const curCity = await _dataHandler__WEBPACK_IMPORTED_MODULE_1__.apiCalls.getCoords(lat, lon);
      apiCords(curCity);
    } catch (err) {
      console.error(err);
    }
  ...
  async function apiCords(city) {
    try {
      ...
      return _viewHandler__WEBPACK_IMPORTED_MODULE_2__.UI.handleDisplay(getWeatherDaysArr, city);
    } catch (err) {
      return err;
    }
  }
  ...
  metricBtn.forEach((btn) =>
    btn.addEventListener("click", (e) => _viewHandler__WEBPACK_IMPORTED_MODULE_2__.UI.checkMetric(e.target))
  );
})();
